...
//
void* LeftLeaningRedBlack::LookUp(unsigned key)
{
...
//	Insert()
//
bool LeftLeaningRedBlack::Insert(VoidRef_t ref, bool* &bref)
{
	m_pRoot = InsertRec(m_pRoot, ref, bref);
...
//
LLTB_t* LeftLeaningRedBlack::InsertRec(LLTB_t* pNode, VoidRef_t ref, bool* &bref)
{
	...
	//
		...
		if (ref.Key < pNode->Ref.Key) {
			pNode->pLeft = InsertRec(pNode->pLeft, ref, bref);
		}
		else {
			pNode->pRight = InsertRec(pNode->pRight, ref, bref);
		}
	}
...
//	Delete()
//
void LeftLeaningRedBlack::Delete(const unsigned key)
{
	if (nullptr != m_pRoot) {
...
//	DeleteRec()
//
LLTB_t* LeftLeaningRedBlack::DeleteRec(LLTB_t *pNode, const unsigned key)
{
	if (key < pNode->Ref.Key) {
		if (nullptr != pNode->pLeft) {
...
{
	if (nullptr != m_pRoot) {
		unsigned  minBlack = 0xFFFFFFFF;
		unsigned  maxBlack = 0;

		// The root of the tree must always be a black node.
		if(false != m_pRoot->IsRed) {
			throw "This isn't the right color";
		}

		SanityCheckRec(m_pRoot, false, 0, minBlack, maxBlack);

		if(minBlack != maxBlack) {
			throw "This violates the rules of the red-black tree";
		}
	}
}
...
//
void LeftLeaningRedBlack::SanityCheckRec(LLTB_t *pNode, bool isParentRed, unsigned blackDepth, unsigned &minBlack, unsigned &maxBlack)
{
	if (pNode->IsRed) {
		// The parent of a red node must be black.
		if(false != isParentRed) {
			throw "This isn't right color. Try again.";
		}
	...
	if ((nullptr != pNode->pLeft) && (nullptr != pNode->pRight)) {
		if (IsRed(pNode->pLeft)) {
			if(false != IsRed(pNode->pRight)) {
				throw "This is the wrong color and violates the red-black tree.";
			}
		}
		if (!IsRed(pNode->pRight)) {
	...
	if (nullptr != pNode->pLeft) {
		...
		if(pNode->pLeft->Ref.Key > pNode->Ref.Key) {
			throw "Error this is impossible got to fix this mess";
		}
	...
	if (nullptr != pNode->pRight) {
		...
		if(pNode->pRight->Ref.Key < pNode->Ref.Key) {
		  throw "Error this is impossible got to fix this mess";
		}
...
//
unsigned LeftLeaningRedBlack::KeyCount(void)
{
...
//	KeyCountRec()
//
unsigned LeftLeaningRedBlack::KeyCountRec(LLTB_t *pNode)
{
	unsigned tally = 0;
...
void LeftLeaningRedBlack::LeafDepth(void)
{
	...
	if (nullptr != m_pRoot) {
		maxDepth = LeafDepthRec(m_pRoot, 0, ary, 64, 0, minBlack, maxBlack);
	}
	...
	for (unsigned  i = 0; i < maxDepth; ++i) {
		...
		cout << i << ":" << ary[i] << "=" << tally << endl;
	}

	cout << "black depth: " << minBlack << "to" << maxBlack << endl;
	...
	if(minBlack != maxBlack) {
		throw "This wrong the statement tells us what to do";
	}
}
...
//	LeafDepthRec()
//
unsigned  LeftLeaningRedBlack::LeafDepthRec(LLTB_t *pNode, unsigned  depth, unsigned  ary[], unsigned  depthLimit, unsigned  blackDepth, unsigned  &minBlack, unsigned  &maxBlack)
{
...
//	TraverseRec()
//
void LeftLeaningRedBlack::TraverseRec(LLTB_t *pNode, unsigned &prev)
{
	if(nullptr == pNode) {
		throw "Must equal pNode which is a root of the node.";
	}
	...
	if(prev > pNode->Ref.Key) {
		throw "This can't be true at all.";
	}
	prev = pNode->Ref.Key;
...
//  Min()
//
unsigned LeftLeaningRedBlack::Min(const unsigned n1, const unsigned n2) {
	if(n1 < n2) {
		return n1;
	}
	else {
...
//  Max()
//
unsigned LeftLeaningRedBlack::Max(const unsigned n1, const unsigned n2) {
	if(n1 > n2) {
		return n1;
	}
	else {
