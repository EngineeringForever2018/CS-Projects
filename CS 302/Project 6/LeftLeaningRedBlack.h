...
public:
	...
	void* LookUp(const unsigned  value);

	bool Insert(VoidRef_t ref, bool* &bref);
	LLTB_t* InsertRec(LLTB_t *pNode, VoidRef_t ref, bool* &bref);

	void Delete(const unsigned  value);
	LLTB_t* DeleteRec(LLTB_t *pNode, const unsigned  value);
	LLTB_t* DeleteMin(LLTB_t *pNode);

	void SanityCheck(void);
	void SanityCheckRec(LLTB_t *pNode, bool isParentRed, unsigned  blackDepth, unsigned  &minBlack, unsigned  &maxBlack);

	unsigned KeyCount(void);
	unsigned KeyCountRec(LLTB_t *pNode);

	void LeafDepth(void);
	unsigned LeafDepthRec(LLTB_t *pNode, unsigned depth, unsigned ary[], unsigned depthLimit, unsigned blackDepth, unsigned &minBlack, unsigned &maxBlack);

	void Traverse(void);
	void TraverseRec(LLTB_t *pNode, unsigned  &prev);

	unsigned Min(const unsigned n1, const unsigned n2);
	unsigned Max(const unsigned n1, const unsigned n2);
